...
const StoryList = () => {
  ...
  const getNews = async () => {
    ...
    const response = await fetch(
      `https://hacker-news.firebaseio.com/v0/${storyFilter}.json?print=pretty`
    );
    ...
    <>
      ...
      >
        <ul className={"hidden sm:flex gap-3"}>
          <li>
            <button
              className={`px-4 py-3 rounded-lg border-2 border-teal-950 ${
                storyFilter === StoryFilter.TopStories ? "bg-teal-900" : ""
              }`}
          ...
          <li>
            <button
              className={`px-4 py-3 rounded-lg border-2 border-teal-950 ${
                storyFilter === StoryFilter.NewStories ? "bg-teal-900" : ""
              }`}
        ...
        <ul
          className={
            "flex flex-wrap gap-3 items-center w-full justify-between sm:w-auto"
          }
        >
          ...
          <li className={"-order-1 sm:order-none"}>
            <button
              className={
                "px-4 py-3 rounded-lg border-2 border-teal-950 disabled:text-zinc-600"
              }
